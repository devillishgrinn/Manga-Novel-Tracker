...
function extractTitleFromMeta(chapter: number): string | null {
  ...
  const withoutSiteSuffix = normalized.replace(/\s*[-|:]\s*Asura\s+Scans\s*$/i, "").trim()

  if (!withoutSiteSuffix || SITE_NAME_REGEX.test(withoutSiteSuffix)) {
    return null
  }

  const titleBeforeChapter = withoutSiteSuffix.match(/^(.*?)\s*[-|:]?\s*Chapter\s+\d+(?:\.\d+)?\b/i)
  ...
  const titleAfterChapter = withoutSiteSuffix.match(/^Chapter\s+\d+(?:\.\d+)?\s*[-|:]\s*(.*)$/i)
...
export const asuraScansAdapter: SiteAdapter = {
  ...
  extract(): TrackerPayload | null {
    ...
    const title =
      extractTitleFromHeading(seriesSlug) || extractTitleFromMeta(chapter) || slugToTitle(seriesSlug)
