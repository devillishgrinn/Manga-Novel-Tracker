...
async function render(): Promise<void> {
  ...
  ;(document.getElementById("unread") as HTMLElement).textContent = unread ? `${unread} unread` : "Up to date"
  if (entries.length === 0) {
    list.innerHTML = '<p class="empty">No tracked series yet. Enable tracking in Settings, then read a supported chapter.</p>'
    return
  }
