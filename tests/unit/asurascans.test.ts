...
describe("asuraScansAdapter", () => {
  ...
  it("match detects Asura chapter URLs across supported domains", () => {
    expect(asuraScansAdapter.match("https://asuracomic.net/series/nano-machine-159d6f56/chapter/299")).toBe(
      true,
    )
    expect(
      asuraScansAdapter.match("https://beta.asurascans.com/series/nano-machine-159d6f56/chapter/299"),
    ).toBe(true)
    expect(asuraScansAdapter.match("https://asuracomic.net/series/nano-machine-159d6f56")).toBe(false)
  })
